"""
Training script for CSDI baseline on traffic / air-quality benchmarks.

Model: model/csdi_traffic.py (adapted from STIMP-release/model/csdi.py)
...
from model.csdi_traffic import IAP_base as CSDI_Base
from utils import check_dir, seed_everything, get_model_size_info, masked_mae, masked_mse


def build_parser():
    p = argparse.ArgumentParser(description='CSDI Traffic Training')
    ...
    # ---- Experiment ----
    p.add_argument('--task_name', type=str, default='csdi-traffic')
    ...
    p.add_argument('--beta_end', type=float, default=0.5)
...
def main():
    ...
    # ---- Model ----
    model = CSDI_Base(config, low_bound, high_bound).to(device)
